...
//
// Initially, the user should just see a screen with the textbox and the "Narrow It Down For Me!" button.
...
// Once retrieved, your task is to loop through all the items and, for each item, do a simple check if the string being searched for by the user
// appears anywhere in the description of the item. If it does, that item gets placed in a special found array. If it doesn't, you simply move on to the next item.
//
...
// In other words, no matter what actions the user has taken, if the "Narrow It Down For Me!" button is clicked, all the data gets wiped out and the whole process starts all over again.
// No requirement to remember what the user chose to throw off the list last time.
...
function NarrowItDownController(MenuSearchService) {
  ...
  narrow.narrowItDown = function() {
    narrow.found = [];
    if(narrow.entry){
      var promise = MenuSearchService.getMatchedMenuItems(narrow.entry);
      promise.then(function (response) {
        narrow.found = response;
        narrow.showMessage = response.length === 0;
      });
    }
  }
...
function MenuSearchService($http, ApiBasePath) {
  ...
  service.getMatchedMenuItems = function (searchTerm) {
    return $http({
      ...
      var foundItems = [];
      results.data.menu_items.forEach(function(item) {
